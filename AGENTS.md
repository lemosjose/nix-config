...
Instructions for AI coding agents (Cursor, GitHub Copilot, etc.) working in this repository.
...
- Commit messages are short, lowercase, area/host-prefixed, e.g. `[tabosa] return to dGPU for GNOME rendering`, `pkgs: add uv and ntfs-3g`, `disko: remove zfs + nixpkgs: new packages`.
- Config is synced across multiple physical machines by hand (git log shows recurring `sync from pc` / `merge diverging setups` commits) — expect and resolve host-specific drift carefully rather than blindly overwriting.
- New shared behavior belongs in `hosts/common/` or `home/common/`; only put things in a host's own `default.nix` when they are genuinely host-specific (hardware quirks, static network config, disk layout).
